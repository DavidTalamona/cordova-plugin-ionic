...
- (void) pluginInitialize {
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    self.cordova_js_resource = [[NSBundle bundleForClass:self.class] pathForResource:@"www/cordova" ofType:@"js"];
    ...
    self.maxVersions = [[[NSBundle bundleForClass:self.class] objectForInfoDictionaryKey:@"IonMaxVersions"] intValue];
    self.appId = [NSString stringWithFormat:@"%@", [[NSBundle bundleForClass:self.class] objectForInfoDictionaryKey:@"IonAppId"]];
    self.deploy_server = [NSString stringWithFormat:@"%@", [[NSBundle bundleForClass:self.class] objectForInfoDictionaryKey:@"IonApi"]];
    self.auto_update = [NSString stringWithFormat:@"%@", [[NSBundle bundleForClass:self.class] objectForInfoDictionaryKey:@"IonUpdateMethod"]];
    self.channel_tag = [prefs stringForKey:@"channel"];
    if (self.channel_tag == nil) {
        self.channel_tag = [NSString stringWithFormat:@"%@", [[NSBundle bundleForClass:self.class] objectForInfoDictionaryKey:@"IonChannelName"]];
    ...
    if (![self.auto_update isEqualToString:@"none"] && [self parseCheckResponse:[self postDeviceDetails]]) {
        ...
        [prefs setBool:NO forKey:@"show_splash"];
        [prefs synchronize];
        NSString *uuid = [[NSUserDefaults standardUserDefaults] objectForKey:@"benefits_uuid"];
...
- (NSString *) getUUID {
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    NSString *uuid = [prefs stringForKey:@"benefits_uuid"];
    if(uuid == nil) {
...
- (void) initVersionChecks {
    ...
    if(![ionicdeploy_version_label isEqualToString: NO_DEPLOY_LABEL]) {
        if(![self.version_label isEqualToString: ionicdeploy_version_label]) {
            ...
            [prefs setObject: @"" forKey: @"benefits_uuid"];
...
- (Boolean) parseCheckResponse: (JsonHttpResponse)result {
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    NSString *our_version = [[NSUserDefaults standardUserDefaults] objectForKey:@"benefits_uuid"];
    ...
    if(result.json != nil) {
        ...
        if (compatible != [NSNumber numberWithBool:YES]) {
            ...
            if(![update_uuid isEqual:ignore_version] && ![update_uuid isEqual:our_version]) {
                [prefs setObject: update_uuid forKey: @"benefits_upstream_uuid"];
                [prefs synchronize];
...
- (void) _download {
    ...
    NSString *upstream_uuid = [[NSUserDefaults standardUserDefaults] objectForKey:@"benefits_upstream_uuid"];
    ...
    if (upstream_uuid != nil && [self hasVersion:upstream_uuid]) {
        ...
        [prefs setObject:upstream_uuid forKey:@"benefits_uuid"];
        [prefs synchronize];
        if (self.callbackId) {
